...
function Profile() {
  ...
  const saveProfile = async () => {
    try{
      if(!username || !avatarData) {
        ...
        await actor.update_profile(user.principal, username, byteArray);
        let avatarUrl = '';
  ...
  return (
    <>
      <div className="flex flex-row py-8 font-plus px-10 gap-8 text-darkblue-800 h-full">
                ...
                (<>
                 <p className=" font-plus font-bold text-18 leading-22">Profile details</p>
                  ...
                  <div className="flex flex-col justify-start w-full gap-[5px]">
                      ...
                      <input className="py-2 pl-4 px-4 rounded-2 w-full font-normal focus:border-transparent focus:outline-none" placeholder="Place of birth" value={placeOfBirth} onChange={(e) => setPlaceOfBirth(e.target.value)}  style={{
                          border: '1.5px solid #e5e7eb', // Set the border color to blue
                          height: '42px', // Adjust the font size as needed
                      }}/>
                  </div>
                  ...
                  <div className="w-full flex flex-row justify-start items-center gap-6">
                    <img className="rounded-[120px] w-40 h-40" src="/demo/assets/artist_profile.png"
                      ...
                      <div className="flex flex-col justify-start items-start">
                        <p className="font-plus-bold text-24">{}</p>
                        <p className="font-plus text-16 font-bold">{user.username}</p>
                        <p className="font-plus text-16 font-bold">Instruments: {}</p>
                      </div>
                  </div>
